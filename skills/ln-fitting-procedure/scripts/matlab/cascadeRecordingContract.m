    ...
    if isfield(meta, 'sample_interval_s')
        ...
        if ~isnumeric(v) || ~isscalar(v) || ~isfinite(v)
            ...
            if rawDt < lo || rawDt > hi
                % Offer the readings but never rank them. A rig that writes MICROSECONDS makes
                % 100 mean 1e-4 s, and an earlier version of this hint named milliseconds and
                % a rate as the two candidates -- both wrong for that rig, and both stated
                % confidently enough to be believed. Whoever recorded it is the only source.
                hint = '';
                if rawDt > hi
                    hint = sprintf([' -- %g is not an interval in seconds. It could be ' ...
                        'milliseconds (%g s), microseconds (%g s), a sample RATE (%g s), or ' ...
                        'another convention entirely; nothing in the file distinguishes them, ' ...
                        'so confirm it with whoever recorded it rather than picking one'], ...
                        rawDt, rawDt/1e3, rawDt/1e6, 1/rawDt);
                end
